...
public class Datacenter extends SimEntity {
	...
	public void processEvent(SimEvent ev) {
		...
		// Resource consumption inquiry
		case CloudSimTagsExt.AVAILABLE_RESOURCE:
			srcId = ((Integer) ev.getData()).intValue();
			List<Host> hostList = getHostList();
			sendNow(srcId, ev.getTag(), hostList);
			break;
		...
		case CloudSimTags.VM_CREATE_ACK:
			// This function is placed in NetworkDatacenter.java class, not this class
	...
	protected void processCloudletMove(int[] receivedData, int type) {
		...
		if (cl == null) {// cloudlet doesn't exist
			...
			if (cl.getCloudletStatus() == Cloudlet.SUCCESS) {// if yes, send it
																// back to user
				int[] data = new int[3];
				data[0] = getId();
				data[1] = cloudletId;
	...
	protected void processCloudletSubmit(SimEvent ev, boolean ack) {
		...
		try {
			...
			if (cl.isFinished()) {
				Log.printLine(TextUtil.toString(CloudSim.clock()) + "[WARN]: DC #" + getId() + ": Warning - Cloudlet #" + cl.getReadableId() + " is already completed/finished.");
	...
	/**
	 * @return a more readable Id of datacenter
	 */
