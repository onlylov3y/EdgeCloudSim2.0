...
public class Cloudlet {
	...
	/**
	 * The size of this Cloudlet to be executed in a CloudResource (unit: in
	 * MI).
	 */
	private long cloudletLength;

	/**
	 * The input file size of this Cloudlet before execution (unit: in byte). in
	 * byte = program + input data size
	 */
	...
	/**
	 * Start time of executing this Cloudlet. With new functionalities, such as
	 * CANCEL, PAUSED and RESUMED, this attribute only stores the latest
	 ...
	 * Allocates a new Cloudlet object. The Cloudlet length, input and output
	 * file sizes should be greater than or equal to 1. By default this
	 * constructor sets the history of this object. The Id of this Cloudlet
	 * Object is automatically generated {@link Id} class.
	 * 
	 * @param cloudletLength
	 *            the length or size (in MI) of this cloudlet to be executed in
	 *            a PowerDatacenter
	 ...
	 * @post $none
	 */
	public Cloudlet(final long cloudletLength, final int pesNumber, final long cloudletFileSize,
			final long cloudletOutputSize, final UtilizationModel utilizationModelCpu,
			final UtilizationModel utilizationModelRam, final UtilizationModel utilizationModelBw, final int userId) {
		this(Id.pollId(Cloudlet.class), cloudletLength, pesNumber, cloudletFileSize, cloudletOutputSize,
	 ...
	 * CloudResource. This Cloudlet length is calculated for 1 Pe only
	 * <tt>not</tt> the total length.
	 * 
	 * @param cloudletLength
	 *            the length or size (in MI) of this Cloudlet to be executed in
	 *            a CloudResource
	...
	public void setResourceParameter(final int resourceID, final double cost) {
		...
		if (index == -1 && record) {
			write("Allocates this Cloudlet to " + res.resourceName + " (ID #" + resourceID + ") with cost = $" + cost
					+ "/sec");
		} else if (record) {
			final int id = resList.get(index).resourceId;
			final String name = resList.get(index).resourceName;
			write("Moves Cloudlet from " + name + " (ID #" + id + ") to " + res.resourceName + " (ID #" + resourceID
					+ ") with cost = $" + cost + "/sec");
		}
	...
	public String getReadableId() {
		String id = "";
		// A Cloudlet owner mis always a SimEntity and can either be a Service
		// or a EdgeDatacenterBroker.
		try {
			id = ((EdgeDatacenterBroker) CloudSim.getEntity(getUserId())).getReadableId() + "." + getCloudletId();
		} catch (Exception e) {
			id = ((Service) CloudSim.getEntity(getUserId())).getReadableId() + "." + getCloudletId();
		}
		return id;
	}
