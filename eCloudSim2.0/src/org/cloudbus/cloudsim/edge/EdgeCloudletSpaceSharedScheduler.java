			...
			if ((cl.getCurrStagenum() != -1)) {
				...
				if (st.getType() == NetworkConstants.EXECUTION) {
					...
					if (cl.getTimespentInStage() >= st.getTime()) {
						CustomLog.printf("%s\t%s\t%s\t%s", CloudSim.clock(), 
								"EXEC VM #" + cl.getVmId(), 
								cl.getTimespentInStage(),
								st.getData());
						// change the stage
								...
								if (pkt.getVirtualrecvid() == cl.getCloudletId()) {
									...
									if (isCloudletOwnerBroker(cl)) {
										...
										CustomLog.printf("%s\t%s\t%s",
												"Service #" + Service.getServiceWithCloudletId(pkt.getVirtualsendid())
														.getId(),
												CloudSim.clock() - cl.getServiceTime(), pkt.getData());

										Service s = Service.getServiceWithCloudletId(pkt.getVirtualsendid());
										if (s.getServiceTyp().equals(ServiceTyp.STREAMING)) {
											CustomLog.printServiceStreaming("%s\t%s\t%s\t%s\t%s\t%s", CloudSim.clock(),
													"#" + s.getReadableId(),
													"#" + s.getRequestBeingProcessed().getNatural(),
													CloudSim.clock() - cl.getServiceTime(), pkt.getData(), "2");
										} else {
											CustomLog.printResponse("%s\t%s\t%s\t%s\t%s", CloudSim.clock(), "#" + s.getReadableId(),
													s.getServiceTyp().getName(), CloudSim.clock() - cl.getServiceTime(),
													pkt.getData());
										}
									...
									Log.printLine(TextUtil.toString(CloudSim.clock()) + " [RECV]: CL #"
											+ cl.getReadableId() + " received a packet from CL #"
											+ pkt.getVirtualsendid() + " with the data: " + pkt.getData());
									CustomLog.printf("%s", "CL #" + cl.getReadableId() + " in VM #" + cl.getVmId()
											+ " [RECV]: from CL #" + pkt.getVirtualsendid());
				...
				if (st.getType() == NetworkConstants.WAIT_SEND) {
					...
					Log.printLine(TextUtil.toString(CloudSim.clock()) + " [SEND]: CL #" + cl.getReadableId()
							+ " in VM #" + cl.getVmId() + " sends packet to CL #" + st.getVpeer() + " in VM #"
							+ st.getPeer() + " with data " + st.getData());
					CustomLog.printf("%s", "CL #" + cl.getReadableId() + " [SEND]: to CL #" + st.getVpeer() + " in VM #"
							+ st.getPeer());
		...
		if (currstage > (cl.getStages().size() - 1)) {
			...
			for (i = cl.getCurrStagenum(); i < cl.getStages().size(); i++) {
				if (cl.getStages().get(i).getType() == NetworkConstants.WAIT_SEND) {
					...
					CustomLog.printf("%s", CloudSim.clock() + "CL #" + cl.getReadableId() + " [SEND]: to CL #"
							+ cl.getStages().get(i).getVpeer() + " in VM #" + cl.getStages().get(i).getPeer());
	...
	public double cloudletSubmit(Cloudlet cloudlet, double fileTransferTime) {
		...
		if ((currentCpus - usedPes) >= cloudlet.getNumberOfPes()) {
			...
			Log.printLine(TextUtil.toString(CloudSim.clock())
					...
					+ cloudlet.getReadableId() + " on VM #" + cloudlet.getVmId() + " current: " + currentCpus
