...
public class EdgeStreamingService extends EdgeService {
	...
	/**
	 * deterministic: 1100 KB
	 */
	private static final double RESPONSE_DATA_SIZE = 1100;
	...
	public void createStages() {
		...
		for (int i = 0; i < cList.size(); i++) {
			...
			if (cl.getCloudletId() == getFirstCloudlet().getCloudletId()) {
				...
				cl.getStages().add(new TaskStage(NetworkConstants.WAIT_RECV, RESPONSE_DATA_SIZE, 0, 3, cl.getMemory(),
						getSecondCloudlet().getVmId(), getSecondCloudlet().getCloudletId()));

				cl.getStages().add(new TaskStage(NetworkConstants.WAIT_SEND, RESPONSE_DATA_SIZE, 0, 4, cl.getMemory(),
						getBrokerVmId(), getBrokerCloudletId()));
			}
			...
			if (cl.getCloudletId() == getThirdCloudlet().getCloudletId()) {
				...
				cl.getStages().add(new TaskStage(NetworkConstants.EXECUTION, 0, 50.0, 1, cl.getMemory(),
						cl.getVmId(), cl.getCloudletId()));
				cl.getStages().add(new TaskStage(NetworkConstants.WAIT_SEND, RESPONSE_DATA_SIZE, 0, 2, cl.getMemory(),
						getSecondCloudlet().getVmId(), getSecondCloudlet().getCloudletId()));
			}
