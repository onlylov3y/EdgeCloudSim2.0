...
public class EdgeDbService extends EdgeService {
	...
	/**
	 * exponential distributed around 50 KB
	 */
	private static final ExponentialRNS RESPONSE_DISTRIBUTION = new ExponentialRNS(1.0 / 50);
	...
	public void createStages() {
		...
		for (int i = 0; i < cList.size(); i++) {
			...
			if (cl.getCloudletId() == getFirstCloudlet().getCloudletId()) {
				...
				cl.getStages().add(new TaskStage(NetworkConstants.WAIT_RECV, REQUEST_DATA_SIZE, 0, 0,
				...
				cl.getStages().add(new TaskStage(NetworkConstants.WAIT_SEND, REQUEST_DATA_SIZE, 0, 2,
						cl.getMemory(), getSecondCloudlet().getVmId(), getSecondCloudlet().getCloudletId()));
				cl.getStages().add(new TaskStage(NetworkConstants.WAIT_RECV, responseSize, 0, 3,
						cl.getMemory(), getSecondCloudlet().getVmId(), getSecondCloudlet().getCloudletId()));

				cl.getStages().add(new TaskStage(NetworkConstants.WAIT_SEND, responseSize, 0, 4,
						cl.getMemory(), getBrokerVmId(), getBrokerCloudletId()));
			}
			...
			if (cl.getCloudletId() == getThirdCloudlet().getCloudletId()) {
				...
				cl.getStages().add(new TaskStage(NetworkConstants.EXECUTION, 0, 50.0, 1, cl.getMemory(),
						cl.getVmId(), cl.getCloudletId()));
				cl.getStages().add(new TaskStage(NetworkConstants.WAIT_SEND, responseSize, 0, 2,
						cl.getMemory(), getSecondCloudlet().getVmId(), getSecondCloudlet().getCloudletId()));
			}
