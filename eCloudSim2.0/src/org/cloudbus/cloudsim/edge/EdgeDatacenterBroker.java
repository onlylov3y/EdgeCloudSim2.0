...
public class EdgeDatacenterBroker extends SimEntity {
	 ...
	 */
	private Map<Integer, List<RequestId>> servicesTorequestIdMap;
	...
	private List<RequestId> streamingVideoLastRequest;

	private List<Vm> vmBeingDestroyed;

	private Map<Integer, Double> videoRequestNaturalIdsToStartTimesMap;
	...
	public EdgeDatacenterBroker(String name, final double lifeLength) throws Exception {
		...
		this.streamingVideoLastRequest = new ArrayList<>();
		this.vmBeingDestroyed = new LinkedList<>();
		this.videoRequestNaturalIdsToStartTimesMap = new HashMap<>();
	...
	public void processEvent(SimEvent ev) {
		...
		case CloudSimTagsExt.BROKER_DESTROY_ITSELF_NOW:
			Log.printLine(TextUtil.toString(CloudSim.clock()) + ": [INFO]: Broker #" + getId()
					+ " TIME TO LIVE reached: processing BROKER_DESTROY_ITSELF_NOW.");
			finishExecution();
			break;
	...
	/**
	 * find the serviceId associated to this serviceId.
	 * 
	 * @param brokerCloudletId
	 *            broker CloudletId
	 * @return return the associated seviceId or -1 if not found
	 */
	public int getServiceIdForBrokerCloudletId(int brokerCloudletId) {
		Map<Integer, Integer> servicesToBrokerCloudlets = getServicesToBrokerCloudletsMap();
		for (Integer serviceId : servicesToBrokerCloudlets.keySet()) {
			if (servicesToBrokerCloudlets.get(serviceId) == brokerCloudletId)
				return serviceId;
		}
		return -1;
	}
	...
	protected void processCloudletReturn(SimEvent ev) {
		...
		RequestId actualRequest = getRequestBeingProcessed();
		if (getStreamingVideoLastRequest().contains(actualRequest)
				&& actualRequest.getTyp().equals(ServiceTyp.STREAMING)) {
			double processingTime = CloudSim.clock()
					- getVideoRequestNaturalIdsToStartTimesMap().get(actualRequest.getNatural());

			int numOfReq = actualRequest.getDecimal();
			double data = 1100.0 * (numOfReq + 1);
			int videoLength = 2 * (numOfReq + 1);

			CustomLog.printResponse("%s\t%s\t%s\t%s\t%s\t%s", CloudSim.clock(),
					"#" + getReadableId() + "." + getServiceIdForBrokerCloudletId(cloudlet.getCloudletId()), "STR",
					processingTime, data, videoLength);
					
			CustomLog.printf("%s\t%s\t%s\t%s", CloudSim.clock(), "Broker #" + getReadableId(),
					"Video #" + getRequestBeingProcessed().getNatural(), processingTime);
		}
		...
		if (getCloudletList().size() == 0 && cloudletsSubmitted == 0) {
			...
			System.out.println(TextUtil.toString(CloudSim.clock()) + ": Broker #" + getId()
	...
	public void processServiceAllCloudletsSent(SimEvent ev) {
		...
		Log.printLine("Amount of services : " + getServiceList().size());
		...
		Log.printLine("Amount of services whose cloudlets were completely sent: " + amount);
	...
	public void processServiceDestroyedItself(SimEvent ev) {
		...
		Log.printLine("Broker #" + getId() + "PROCESSING SERVICE_DESTROYED_ITSELF FROM SERVICE #" + serviceId);
	...
	protected void processBrokerMessage(SimEvent ev) {
		...
		RequestId requestId = (RequestId) data[0];
		int serviceId = (int) data[1];
		Message msg = (Message) data[2];
		...
		if (this.getLifeLength() > 0 && CloudSim.clock() > this.getLifeLength()) {
			...
			boolean RequestIsNext = this.getServicesTorequestIdMap().get(serviceId).get(0).equals(requestId);
			if (!isServiceProcessing && containsBrokerId && serviceHasCloudletMapping && RequestIsNext) {
				// set this request as the one being processed
				setRequestBeingProcessed(requestId);
				// this is a Streaming request and it is the first chunk
				if (requestId.isFirst() && requestId.getTyp().equals(ServiceTyp.STREAMING)) {
					getVideoRequestNaturalIdsToStartTimesMap().put(requestId.getNatural(), CloudSim.clock());
				}
				...
				Object[] dataToSend = {msg,requestId};
				sendNow(serviceId, CloudSimTagsExt.BROKER_MESSAGE, dataToSend);
			} else {
	...
	/**
	 * Start the Service without submitting Cloudlet
	 */
	...
	public void startService(int serviceId) {
		...
		System.out.println("[STARTING SERVICE]: " + TextUtil.toString(CloudSim.clock()) 
						+ " Service # " + getReadableId() + "." + serviceId 
						+ " is being started...");
	...
	protected void processOtherEvent(SimEvent ev) {
		...
		case CloudSimTagsExt.SERVICE_CLOUDLET_DONE:
			Log.printLine(TextUtil.toString(CloudSim.clock()) + ": Broker #" + getId() + ": Service #"
	...
	/**
	 * <pre>
	 * 1) Process the ack received due to a request for UserVM creation in UserDC
	 * 2) Start to request the Datacenter for available resources
	 * </pre>
	 * @param ev
	 *            a SimEvent object
	 * @pre ev != null
	...
	protected void processVmCreate(SimEvent ev) {
		...
		if (result == CloudSimTags.TRUE) {
			...
			po.clearFreeHostList();
					
			// Starting request Datacenter for available resources
			schedule(serviceId, 0, CloudSimTagsExt.AVAILABLE_RESOURCE_REQUEST);
			
			// UserVM that has been created in datacenter			
			Vm userVm = VmList.getById(getVmsCreatedList(), vmId);
			// notify the service to start creating its VMs in Datacenters			
			send(serviceId, 1.0, CloudSimTagsExt.SERVICE_SUBMIT_VMS_NOW, userVm);
	...
	public void createStages(int serviceId, Message msg) {
		...
		Log.printLine(TextUtil.toString(CloudSim.clock()) + ": [INFO]: Broker #" + getId()
				...
				+ " and service 1st CL VM #" + firstVmId + " Cloudlet #" + cloudlet.getReadableId() + " and Message "
		...
		Log.printLine(TextUtil.toString(CloudSim.clock()) + ": [INFO]: Broker #" + getId() + " Cloudlet #"
				+ cloudlet.getReadableId() + " has " + cloudlet.getStages().size() + " Stages");
	...
	protected void submitCloudlets() {
		...
		for (int i = 0; i < getCloudletList().size(); i++) {
			...
			if (vm == null || getVmBeingDestroyed().contains(vm)) {
				Log.printLine(TextUtil.toString(CloudSim.clock()) + ": Broker #" + getId()
			...
			// sendNow(getUserDC().getId(), CloudSimTags.CLOUDLET_SUBMIT,
			// cloudlet);
			cloudletsSubmitted++;
	...
	public void submitCloudlet(Cloudlet cloudlet) {
		...
		Log.printLine(TextUtil.toString(CloudSim.clock()) + ": Broker #" + getId()
				+ ": called submitCloudlet with Cloudlet #" + cloudlet.getReadableId());

		vm = VmList.getById(getVmsCreatedList(), vmId);
		if (vm == null || getVmBeingDestroyed().contains(vm)) {
			Log.printLine(
					TextUtil.toString(CloudSim.clock()) + ": Broker #" + getId() + ": Postponing execution of cloudlet "
							+ cloudlet.getReadableId() + ": bount VM #" + cloudlet.getVmId() + " not available");
		} else {
			Log.printLine(TextUtil.toString(CloudSim.clock()) + ": Broker #" + getId() + ": Sending cloudlet #"
					+ cloudlet.getReadableId() + " to VM #" + vmId);
		...
		if (result == CloudSimTags.TRUE) {
			...
			for (Cloudlet cloudlet : getCloudletSubmittedList()) {
				if (!cloudlet.isFinished() && vmId == cloudlet.getVmId()) {
					Log.printLine(CloudSim.clock() + ": Broker #" + getId() + " TRYING TO TERMINATE CLOUDLET #"
					...
					try {
						...
						Log.printLine(CloudSim.clock() + ": Broker #" + getId() + Level.SEVERE + e.getMessage()
								+ e.toString());
					}
	...
	public void addRequestId(int serviceId, RequestId requestId) {
		List<RequestId> ids;
	...
	public void sortRequestIdList() {
		for (Integer key : getServicesTorequestIdMap().keySet()) {
			Collections.sort(getServicesTorequestIdMap().get(key));
		}

	}

	public Map<Integer, List<RequestId>> getServicesTorequestIdMap() {
	...
	/**
	 * add a the last request of a video to the list.
	 * 
	 * @param rId
	 *            the request to add
	 */
	public void addVideoLastRequest(RequestId rId) {
		this.getStreamingVideoLastRequest().add(rId);
	}

	/**
	 * @return the streamingVideoLastRequest
	 */
	public List<RequestId> getStreamingVideoLastRequest() {
		return streamingVideoLastRequest;
	}
	...
	/**
	 * @return the vmBeingDestroyed
	 */
	public List<Vm> getVmBeingDestroyed() {
		return vmBeingDestroyed;
	}

	/**
	 * @return the videoRequestStartTimes
	 */
	public Map<Integer, Double> getVideoRequestNaturalIdsToStartTimesMap() {
		return videoRequestNaturalIdsToStartTimesMap;
	}
