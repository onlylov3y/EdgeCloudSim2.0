...
public class NetworkDatacenter extends Datacenter {
	...
	public boolean processVmCreateNetwork(Vm vm) {
		...
		if (result) {
			...
			Log.printLine(TextUtil.toString(CloudSim.clock()) + ": Datacenter #" + getId() + " - VM #" + vm.getId()
	...
	public void processVmCreate(SimEvent ev, boolean ack) {
		...
		if (ack) {		
			int[] data = new int[3];
			// Datacenter ID - can be either UserDC or Datacenter
			data[0] = getId();
			// VM id to be created
			data[1] = vm.getId();
			if (result) {
				data[2] = CloudSimTags.TRUE;
//				System.out.println(TextUtil.toString(CloudSim.clock()) + "[DATACENTER]: Service #" + getReadableId() + ": VM #" + vm.getId()
//						+ " created in Datacenter #" + getId() + ", Host #" + vm.getHost().getReadableId());				
			} else {
				data[2] = CloudSimTags.FALSE;
			}
			// If vm is UserVm, then send CloudSimTags.VM_CREATE_ACK to EdgeDatacenterBroker class
			// to create one UserVM in UserDC
			// Else vm is a service chain VM, then send CloudSimTags.VM_CREATE_ACK to Service class
			// to create service chain VMs in Datacenter. In this case, there will be 3 times calling
	...
	public void startEntity() {
		...
		Log.printLine(TextUtil.toString(CloudSim.clock()) + ": Datacenter " + typ + " #" + getId() + " - " + getName()
				+ " is starting...");
