...
public class Switch extends SimEntity {
	...
	public void startEntity() {
		...
		Log.printLine(TextUtil.toString(CloudSim.clock()) + ": Switch " + level + " #" + getId() + " - " + getName()
				+ " is starting...");
	...
	protected void processpacketforward(SimEvent ev) {
		...
		if (downlinkswitchpktlist != null) {
			for (Entry<Integer, List<NetworkPacket>> es : downlinkswitchpktlist.entrySet()) {
				...
				if (!hspktlist.isEmpty()) {
					bw = NetworkTopology.isNetworkEnabled() ? NetworkTopology.getBw(getId(), tosend)
							: downlinkbandwidth;
					avband = bw / hspktlist.size();
					Iterator<NetworkPacket> it = hspktlist.iterator();
					while (it.hasNext()) {
						NetworkPacket hspkt = it.next();
						double delay = hspkt.pkt.data / (avband * 1000);

						CustomLog.printf("%s\t%s\t%s\t%s\t%s", CloudSim.clock(), 
								"#" + this.getId() + "->#" + tosend,
		...
		if (uplinkswitchpktlist != null) {
			for (Entry<Integer, List<NetworkPacket>> es : uplinkswitchpktlist.entrySet()) {
				...
				if (!hspktlist.isEmpty()) {
					...
					bw = NetworkTopology.isNetworkEnabled() ? NetworkTopology.getBw(getId(), tosend) : uplinkbandwidth;

					avband = bw / hspktlist.size();
					Iterator<NetworkPacket> it = hspktlist.iterator();
					while (it.hasNext()) {
						NetworkPacket hspkt = it.next();
						// conversion from s to ms (KBps to KBpms)
						double delay = hspkt.pkt.data / (avband * 1000);
		...
		if (packetTohost != null) {
			for (Entry<Integer, List<NetworkPacket>> es : packetTohost.entrySet()) {
				List<NetworkPacket> hspktlist = es.getValue();
				if (!hspktlist.isEmpty()) {
					// The host is no CloudSim Entity, reason why there is no
					// information
					// to in the BRITE file. The link from ESwitch <-> DC in
					// the BRITE file represents the link ESwitch <-> Host
					bw = NetworkTopology.isNetworkEnabled() ? NetworkTopology.getBw(getId(), dc.getId())
							: downlinkbandwidth;
					avband = bw / hspktlist.size();
					// avband = downlinkbandwidth / hspktlist.size();
					Iterator<NetworkPacket> it = hspktlist.iterator();
					while (it.hasNext()) {
						NetworkPacket hspkt = it.next();
						// conversion from s to ms (KBps to KBpms)
	...
	public NetworkDatacenter getDc() {
		return dc;
	}

	public void setDc(NetworkDatacenter dc) {
		this.dc = dc;
	}
